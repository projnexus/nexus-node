...
export const getGuild = async function(this: Client, guildId: string): Promise<{ response: Response; status: number }> {
    if (!this) throw new Error('Client not initialized');
...
export const deleteGuild = async function(this: Client, guildId: string): Promise<{ response: Response; status: number }> {
    if (!this) throw new Error('Client not initialized');
